...
class MohrUserProvider implements UserProvider
{
    /**
    ...
    public function retrieveById($identifier)
    {
        ...
        return $model->newQuery()->RefsAnggota()
    ...
    public function retrieveByToken($identifier, $token)
    {
        ...
        $model = $model->RefsAnggota()->where($model->getAuthIdentifierName(), $identifier)->first();
    ...
    public function retrieveByCredentials(array $credentials)
    {
        ...
        foreach ($credentials as $key => $value) {
            ...
            if (is_array($value) || $value instanceof Arrayable) {
                $query->RefsAnggota()->whereIn($key, $value);
            } else {
                $query->RefsAnggota()->where($key, $value);
            }
        }

        return $query->RefsAnggota()->first();
    }
    ...
    public function validateCredentials(UserContract $user, array $credentials)
    {
        ...
        return (strcmp($username, $user->{$user->username()}) === 0) && $this->hasher->check($plain, $user->getAuthPassword());
    }
